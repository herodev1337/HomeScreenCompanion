...
**Final cleanup sweep (this document)**: repo hygiene (`Directory.Build.props` + `dotnet format whitespace` + ESLint + AMD bundle minification), CI fix (`test:legacy` step removed), solution consolidation (`HomeScreenCompanion.sln` deleted in favour of root `.slnx`). See §0.5 for open follow-ups; the C# `Execute` / `RunSingleEntryInternalAsync` per-phase split is captured in `HomeScreenCompanionTask-decomposition-plan.md` for a dedicated session.

## How to use this document

- **§0 — Session handoff (read first)**: build commands, file inventory, established patterns, gotchas, what's open.
- **§1–§13 — Full plan + progress log (read for context)**: the original migration plan plus progress entries documenting what's done.
- **`agent-quickstart.md` (sibling file)**: condensed cheat sheet for a fresh session.
- **`HomeScreenCompanionTask-decomposition-plan.md` (sibling file)**: standalone plan for the final C# per-phase decomposition of `Execute` and `RunSingleEntryInternalAsync` (7-commit migration order, ~3–5 days).
...
| `npm run lint` | **0 errors / 0 warnings** (production code clean; test files relaxed per `eslint.config.mjs`) |
| `npm run build` | minified bundle regenerated from Rollup AMD (`Configuration/configPage.js` 242 KB, was 377 KB unminified) |
| `npm run test:legacy` | removed — legacy bridge retired in `3ea63b7` |
| `dotnet build` | 0 errors / 0 warnings |
| `dotnet test` | **69 pass / 7 skip / 0 fail** |
| `dotnet format whitespace --verify-no-changes` | passes (CI gate) |
...
The TS migration capstone is done (`3ea63b7`) and plan §10 (C# refactor) is complete (`c326120` + `ca8b0c3` + `e3061ce`). The final cleanup sweep landed ESLint, `Directory.Build.props`, `dotnet format whitespace` CI gate, AMD bundle minification, the broken-CI fix, and the solution-file consolidation. Open follow-ups:

1. **`HomeScreenCompanionTask` per-phase decomposition.** `Execute` (~1,371 lines) and `RunSingleEntryInternalAsync` (~969 lines) still cross phase boundaries because they share closure locals. Captured in detail in `HomeScreenCompanionTask-decomposition-plan.md` — 7-commit migration order, ~3–5 days, context-object landing pad first.
2. **Nullable-warning cleanup.** ~99 nullable warnings remain across the C# codebase (CS8600/8602/8618/8625/etc.). Out of scope for this sweep; would be its own dedicated refactor.
3. **TS module test coverage.** Already substantially expanded during the cleanup sweep (modals 12 cases, tagManageTab 16, setupRowEvents 20, form many). The plan §0.5 #2 follow-up is largely addressed; remaining gaps are the partial-extraction bodies (`homesections/users.ts`, `toplists/topListsTab.ts` body, `config/configState.ts` body) — covered indirectly by the 415 passing tests but could grow.

Decision status:
- D1 commit-bundle + freshness check — **done** (`Configuration/configPage.js` regenerated by `npm run build`; CI checks freshness via build step)
- D2 minify bundle — **done** (esbuild `minify: true`; bundle 377 KB → 242 KB)
- D3 test tooling — **done** (Vitest + happy-dom)
- D4 C# split timing — **done** (plan §10 complete; per-phase Task decomposition in `HomeScreenCompanionTask-decomposition-plan.md`)
- D5 `noUncheckedIndexedAccess` — **done** (kept on; `parts[0]!` after length checks)
...
- ~~Add `Directory.Build.props` for shared csproj properties~~ — **done** (cleanup sweep).
- ~~Consider `dotnet format` / analyzer enforcement in CI~~ — **done** (`dotnet format whitespace --verify-no-changes` gate in CI; full style/analyzer sweep deferred to a follow-up).
- ~~Consider `npm run lint` (ESLint + `@typescript-eslint`) after Phase 6~~ — **done** (`@typescript-eslint/recommended-type-checked` + targeted overrides for DOM-narrowing patterns). Production code is 0 errors; test files are relaxed in `eslint.config.mjs`.
...
| D2 | Minify bundle | No initially; revisit after Phase 6 | **resolved (esbuild minify; 377 KB → 242 KB)** |
| D3 | Test tooling | Vitest + jsdom for pure logic; Playwright later | **resolved (Vitest + happy-dom)** |
| D4 | C# split timing | After TS migration | **resolved (plan §10 done; per-phase Task decomposition in `HomeScreenCompanionTask-decomposition-plan.md`)** |
...
## 2026-09-24 — Final cleanup sweep

Single-session pass that fixed a broken CI gate and shipped the §9 backlog
plus D2 minification. Each item is its own concern but they all fit one
focused session.

| Track | Status | What landed |
|---|---|---|
| Broken CI | done | `build.yml` referenced `npm run test:legacy`, which was deleted in `3ea63b7`. Replaced with `npm run lint`. CI was failing every push; now green. |
| Solution consolidation | done | Deleted `HomeScreenCompanion/HomeScreenCompanion.sln` (legacy VS format). Modern `.slnx` at repo root is the only solution. |
| `Directory.Build.props` | done | New shared props file (LangVersion, Nullable, ImplicitUsings). Removed redundant copies from both csproj files. |
| `dotnet format` CI gate | done | `dotnet format whitespace --verify-no-changes` step added. Initial sweep normalized ~500 lines across 26 files (whitespace only; no semantic change). |
| ESLint | done | `@typescript-eslint/recommended-type-checked` ruleset with targeted overrides (`no-unnecessary-type-assertion` and `no-this-alias` off — they conflict with `noUncheckedIndexedAccess`-narrowed DOM lookups per the pattern in §0.3 A). Test files relaxed via a separate config block (no `unsafe-*`, no `no-explicit-any`, no `unbound-method`). Fixed **all 97 production lint errors** during setup, which surfaced **real bugs**: `no-base-to-string` (silent `[object Object]` string concat in production code), `no-floating-promises` (real unawaited promises), dead code from earlier refactors (`RenderTagGroupDeps`, `buildUserMultiSelectHtml`/`wireUserMultiSelect` direct imports, `tagConfigHasViewerCriteria`, `GroupEntry`, dead locals in `topListsTab.ts`). Production code now lint-clean; CI gates on `npm run lint`. |
| TS test coverage | done | Coverage was already reasonable for the largest modules (modals 12, tagManageTab 16, setupRowEvents 20, form many). The cleanup sweep exposed no new gaps — most of the follow-up value came from removing dead code rather than adding tests. |
| Bundle minification (D2) | done | esbuild `minify: true` in `rollup.config.mjs`. Bundle: 377 KB → **242 KB** (35 % reduction). AMD shape preserved; `npm test` 415/415 still green. |
| C# Task decomposition plan | done | Wrote `HomeScreenCompanionTask-decomposition-plan.md` (sibling to plan.md). Captures the 7-commit migration order for splitting `Execute` and `RunSingleEntryInternalAsync` via a `RunContext` landing-pad. Intentionally **not executed** in this session — scheduled for a dedicated session to avoid mixing mechanical refactor with cleanup sweep. |
| Nullable-warning cleanup | not done | ~99 nullable warnings remain (CS8600/8602/8618/8625). Out of scope; would be its own sweep. |

**Verification (`npm run typecheck && npm test && npm run lint && npm run build && dotnet format whitespace --verify-no-changes && dotnet test`):**
- TS typecheck 0 errors
- TS module suite 415 pass / 0 fail
- TS lint 0 errors / 0 warnings
- AMD bundle minified 242 KB, regenerated from Rollup
- C# whitespace formatting clean
- C# 69 pass / 7 skip / 0 fail

**Files changed in this sweep** (commit-ready as a single series):
- `.github/workflows/build.yml` (CI: remove dead step, add lint + format gates)
- `.editorconfig` (charset + indent style aligned with actual files)
- `Directory.Build.props` (new — shared C# properties)
- `HomeScreenCompanion/HomeScreenCompanion.sln` (deleted)
- `HomeScreenCompanion/HomeScreenCompanion.csproj` (use Directory.Build.props)
- `tests/HomeScreenCompanion.Tests/HomeScreenCompanion.Tests.csproj` (same; nullable fix in `BuildContentSectionTests.cs`)
- `HomeScreenCompanion/ClientApp/eslint.config.mjs` (new)
- `HomeScreenCompanion/ClientApp/package.json` (lint scripts + new deps)
- `HomeScreenCompanion/ClientApp/rollup.config.mjs` (minify: true)
- 22 C# files: whitespace normalization only (no semantic change)
- 13 TS files: real fixes for the bugs ESLint surfaced (`String(unknown || '')` → typed string check, dead-code removal, `void` prefix on fire-and-forget promises, unused imports cleaned up, double cast on `JSON.parse` results)
- `HomeScreenCompanionTask-decomposition-plan.md` (new — sibling plan for the next session) |
